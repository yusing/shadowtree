...
`fix`, `fmt`, and `tidy` are unsandboxed by default. Module-wide Go built-ins use `for_each = "@go-modules"` and `workdir = "{item}"`; the `./...` package pattern is evaluated inside each module directory, not at the repo root. Package-style Go built-ins also expose an optional positional `pkg` argument for shell completion from `@go-packages`; `fix` is available when the most common `go.mod` directive is greater than `1.26`; `fmt` exposes an optional positional `target` from `@go-packages` plus `@glob "*.go"`. `run` has a required positional `command` argument with `rel_path` type and completes from `@go-main-packages` plus `@glob "*.go"`. Project config can override any built-in recipe field; partial overrides preserve unspecified built-in fields.
