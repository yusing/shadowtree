# Summary

[Shadowtree](README.md)

- [Getting Started](getting-started.md)
- [Sandboxing and Sync-Out](sandboxing-and-sync-out.md)
- [Global Flags](global-flags.md)
- [CLI Inspection](cli-inspection.md)
- [Shell Completion](shell-completion.md)

# Configuration

- [Configuration Files](configuration.md)
- [Includes](includes.md)
- [Variables and Environment](variables-and-env.md)
- [Shell Commands](shell-commands.md)
- [Placeholders](placeholders.md)
- [Recipe References](recipe-references.md)
- [Recipe Resolution](recipe-resolution.md)

# Recipes

- [Recipe Fields](recipes.md)
- [Recipe Lifecycle](recipe-lifecycle.md)
- [Fan-Out and Workdirs](fan-out-and-workdirs.md)
- [Recipe Logging](recipe-logging.md)
- [Tool Requirements](tool-requirements.md)
- [Reserved Names](reserved-names.md)

# Arguments

- [Typed Arguments](typed-arguments.md)
- [Value Providers](value-providers.md)
- [Variadic Args](variadic-args.md)
- [Presets](presets.md)

# Profiles

- [Profile Selection](built-in-profiles.md)
- [Go Profile](go-profile.md)
- [Node Profile](node-profile.md)

# Integrations

- [Editor Support](editor-support.md)
- [Development](development.md)

# Reference

- [Behavior Spec](reference/spec.md)
- [System Container Sandbox Pipeline](reference/persistent-sandbox-pipeline.md)
- [Known Limits](known-limits.md)
